...
export class Produit2Component implements OnInit {
  ...
  // ----------------------------------------------------------------
  // Galerie : une colonne de vues sur grand écran, un carrousel aimanté
  // en dessous. Les deux dispositions sont la même liste, seule la feuille
  // de style change d'axe — d'où un seul mécanisme de navigation ici.
  // ----------------------------------------------------------------

  private readonly shotRefs = viewChildren<ElementRef<HTMLElement>>('shot');

  /**
   * Amène une vue sous les yeux. `block: 'nearest'` plutôt que `'start'` : en
   * colonne, la vue voisine est souvent déjà visible en partie, et un
   * défilement qui la recale en haut de fenêtre donnerait un sursaut pour
   * rien. `inline: 'center'` sert au carrousel horizontal.
   */
  private bringViewIntoSight(index: number): void {
    const element = this.shotRefs()[index]?.nativeElement;
    if (!element) {
      return;
    }
  ...
  /**
   * Le rail marque la vue courante. Sans cet observateur il mentirait dès que
   * le carrousel est parcouru au doigt : le rang ne bougerait qu'au clic, alors
   * que la vue affichée, elle, a changé.
   *
   * Le seuil est haut (60 %) parce que deux vues se chevauchent pendant tout
   * le défilement : en deçà, le marquage sauterait d'une vignette à l'autre au
   * milieu du geste.
   *
   * L'effet suit `shotRefs` : la galerie n'existe qu'une fois le produit chargé
   * et elle est reconstruite au passage d'une déclinaison à l'autre. Poser les
   * cibles une seule fois laisserait l'observateur sur des nœuds détachés.
   */
  private readonly trackVisibleShot = effect((onCleanup) => {
    const shots = this.shotRefs();
    if (shots.length === 0 || typeof IntersectionObserver === 'undefined') {
      return;
    }
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          ...
          const index = shots.findIndex((ref) => ref.nativeElement === entry.target);
          if (index !== -1) {
            this.selectedViewIndex.set(index);
          }
        }
    ...
    for (const shot of shots) {
      observer.observe(shot.nativeElement);
    }
  ...
  private flashAdded(): void {
    ...
    this.addedTimer = setTimeout(
      ...
      Produit2Component.ADDED_FLASH_MS,
    );
  }
  ...
  private load(slug: string): void {
    ...
    this.shopService.findBySlug(slug).subscribe({
      next: (product) => {
        ...
        // Voir la note de `Boutique2Component.ngOnInit`.
